...
const __chunk_6 = require('./chunk-7994e8a0.js');
const __chunk_8 = require('./chunk-9837eb0d.js');
...
const createScrollHistory = (win, applicationScrollKey = 'scrollPositions') => {
    ...
    const set = (key, value) => {
        ...
        if (__chunk_8.storageAvailable(win, 'sessionStorage')) {
            const arrayData = [];
...
const createBrowserHistory = (win, props = {}) => {
    ...
    const canUseHistory = __chunk_8.supportsHistory(win);
    const needsHashChangeListener = !__chunk_8.supportsPopStateOnHashChange(globalNavigator);
    const scrollHistory = createScrollHistory(win);
    const forceRefresh = (props.forceRefresh != null) ? props.forceRefresh : false;
    const getUserConfirmation = (props.getUserConfirmation != null) ? props.getUserConfirmation : __chunk_8.getConfirmation;
    const keyLength = (props.keyLength != null) ? props.keyLength : 6;
    const basename = props.basename ? __chunk_6.stripTrailingSlash(__chunk_6.addLeadingSlash(props.basename)) : '';
    ...
    const getDOMLocation = (historyState) => {
        ...
        warning((!basename || __chunk_6.hasBasename(path, basename)), 'You are attempting to use a basename on a page whose URL path does not begin ' +
            'with the basename. Expected path "' + path + '" to begin with "' + basename + '".');
        if (basename) {
            path = __chunk_6.stripBasename(path, basename);
        }
        return __chunk_6.createLocation(path, state, key || __chunk_6.createKey(keyLength));
    };
    ...
    const handlePopState = (event) => {
        ...
        if (!__chunk_8.isExtraneousPopstateEvent(globalNavigator, event)) {
    ...
    const createHref = (location) => {
        return basename + __chunk_6.createPath(location);
    };
    const push = (path, state) => {
        warning(!(typeof path === 'object' && path.state !== undefined && state !== undefined), 'You should avoid providing a 2nd state argument to push when the 1st ' +
            'argument is a location-like object that already has state; it is ignored');
        const action = 'PUSH';
        const location = __chunk_6.createLocation(path, state, __chunk_6.createKey(keyLength), history.location);
        transitionManager.confirmTransitionTo(location, action, getUserConfirmation, (ok) => {
            if (!ok) {
                return;
            }
...
const HashPathCoders = {
    hashbang: {
        encodePath: (path) => path.charAt(0) === '!' ? path : '!/' + __chunk_6.stripLeadingSlash(path),
        decodePath: (path) => path.charAt(0) === '!' ? path.substr(1) : path
    },
    noslash: {
        encodePath: __chunk_6.stripLeadingSlash,
        decodePath: __chunk_6.addLeadingSlash
    },
    slash: {
        encodePath: __chunk_6.addLeadingSlash,
        decodePath: __chunk_6.addLeadingSlash
    }
};
const createHashHistory = (win, props = {}) => {
    ...
    const globalLocation = win.location;
    const globalHistory = win.history;
    const canGoWithoutReload = __chunk_8.supportsGoWithoutReloadUsingHash(win.navigator);
    const keyLength = (props.keyLength != null) ? props.keyLength : 6;
    const { getUserConfirmation = __chunk_8.getConfirmation, hashType = 'slash' } = props;
    const basename = props.basename ? __chunk_6.stripTrailingSlash(__chunk_6.addLeadingSlash(props.basename)) : '';
    ...
    const getDOMLocation = () => {
        ...
        return __chunk_6.createLocation(path, undefined, __chunk_6.createKey(keyLength));
    };
    const transitionManager = createTransitionManager();
    const setState = (nextState) => {
        Object.assign(history, nextState);
    ...
    const handleHashChange = () => {
        ...
        if (path !== encodedPath) {
            ...
            if (!forceNextPop && __chunk_6.locationsAreEqual(prevLocation, location)) {
                return; // A hashchange doesn't always == location change.
            }
            if (ignorePath === __chunk_6.createPath(location)) {
    ...
    const revertPop = (fromLocation) => {
        ...
        let toIndex = allPaths.lastIndexOf(__chunk_6.createPath(toLocation));
        let fromIndex = allPaths.lastIndexOf(__chunk_6.createPath(fromLocation));
        if (toIndex === -1) {
            toIndex = 0;
        }
        if (fromIndex === -1) {
    ...
    let allPaths = [__chunk_6.createPath(initialLocation)];
    // Public interface
    const createHref = (location) => ('#' + encodePath(basename + __chunk_6.createPath(location)));
    const push = (path, state) => {
        warning(state === undefined, 'Hash history cannot push state; it is ignored');
        const action = 'PUSH';
        const location = __chunk_6.createLocation(path, undefined, __chunk_6.createKey(keyLength), history.location);
        transitionManager.confirmTransitionTo(location, action, getUserConfirmation, (ok) => {
            if (!ok) {
                return;
            }
            const path = __chunk_6.createPath(location);
            ...
            if (hashChanged) {
                ...
                const prevIndex = allPaths.lastIndexOf(__chunk_6.createPath(history.location));
    ...
    const replace = (path, state) => {
        ...
        transitionManager.confirmTransitionTo(location, action, getUserConfirmation, (ok) => {
            ...
            const prevIndex = allPaths.indexOf(__chunk_6.createPath(history.location));
            if (prevIndex !== -1) {
