import { Component, Prop, Element, Event, Method, Watch, h, State } from '@stencil/core';
...
export class Switch {
    ...
    render() {
        return (h("label", { class: "label", htmlFor: this.name, onClick: () => { this.activate(); } },
            h("input", { type: "checkbox", name: this.name, id: this.name, checked: this.checked, tabindex: "-1", onClick: () => { this.activate(); } }),
            h("button", { type: "button" },
                h("span", null,
                    h("ion-icon", { name: this.checked ? "checkmark" : "close" }))),
            h("slot", null)));
    }
