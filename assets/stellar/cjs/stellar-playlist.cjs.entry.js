...
const Playlist = class {
    constructor(hostRef) {
        ...
        this.load_songs = index.createEvent(this, "load_songs", 7);
    }
    componentWillLoad() {
    ...
    handleUpdates() {
        this.audio.addEventListener("update", () => {
            this.currentTime = index$1$1.asTime(parseFloat(this.audio.currentTime.toString()).toFixed(10));
            this.duration = index$1$1.asTime(parseFloat(this.audio.duration.toString()).toFixed(10));
    ...
    render() {
        return (index.h("div", { id: "player" }, index.h("div", { class: "playlist-title" }, index.h("h6", null, this.name), index.h("button", { class: "playlist", onClick: () => this.togglePlaylist() }, index.h("h6", { class: "list" }, index.h("ion-icon", { name: "musical-notes" }), "list")), index.h("button", { class: "switch-view", onClick: () => this.toggleAlbumArtView(), "data-playing": this.playing }, index.h("h6", { class: "list" }, (this.view === "art")
            ? index.h("ion-icon", { name: "albums" })
            : index.h("ion-icon", { name: "radio" }), (this.view === "art")
            ? "Art"
            : "Player")), index.h("stellar-color-picker", { val: this.visualizationColor, notransparent: true, onUpdate: (e) => { this.visualizationColor = e.detail; } })), index.h("div", { class: "playlist-playing" }, index.h("button", { onClick: () => { this.handlePlay(); }, class: "toggle-button", "data-playing": this.playing }, (this.playing)
            ? index.h("ion-icon", { name: "pause" })
            : index.h("ion-icon", { name: "play" })), !this.load && index.h("stellar-button", { tag: "button", size: "tiny", onClick: () => { this.load = true; this.load_songs.emit({}); } }, "Load ", this.name || "this playlist"), this.load && index.h("div", { class: "playlist-playing-details" }, index.h("h2", null, this.currentTrack.title || 'Loading...'), index.h("h3", null, this.currentTrack.artist || 'One Sec...')), this.load &&
            index.h("div", { class: "playlist-playing-image" }, this.loading && index.h("stellar-progress", { indeterminate: true }), !this.loading && (this.currentTrack.picture !== undefined) && index.h("img", { src: this.currentTrack.picture, onClick: () => this.toggleAlbumArtView() })), (!this.load && this.currentTrack.picture === undefined) && index.h("skeleton-img", { width: 1024, height: 1024, block: true, loading: true }), index.h("web-audio-visualizer", { type: this.visualizationType, color: this.visualizationColor, onClick: () => { this.cycleVisualizations(); }, width: 1024, height: this.view === "art" ? 1024 : 256 }), index.h("div", { class: `playlist-progress ${this.load ? "load" : "noload"}` }, index.h("h6", { id: "currentTime" }, this.currentTime), index.h("h6", { id: "totalTime" }, this.duration), index.h("progress", { id: "progress", max: "100", value: this.progressValue }))), index.h("div", { id: "controls", class: "controls" }, index.h("button", { onClick: () => this.previous(), class: "button previous" }, index.h("ion-icon", { name: "skip-backward" })), index.h("button", { onClick: () => this.next(), class: "button next" }, index.h("ion-icon", { name: "skip-forward" }))), index.h("audio", { id: "playlist-audio", preload: "auto", tabindex: "0", controls: true }, index.h("source", { id: "source", type: "audio/mp3" })), index.h("div", { id: "playlist", class: `playlist-list ${this.playlist}` }, this.load && index.h("slot", null))));
    }
