...
const Picture = class {
    ...
    mountZoom() {
        ...
        this.zoom = index$1$1.mediumZoom(zoomable, {
            background: `${this.bg}`,
            scrollOffset: 1,
            margin: 30
        });
    ...
    renderPicture() {
        if (this.active) {
            return [
                this.sources.map((source) => index.h("source", { srcSet: source.srcset, media: source.media ? source.media : false })),
                index.h("img", { src: this.large, class: "final_image", alt: this.alt })
            ];
        }
    }
    render() {
        if (this.type === "picture") {
            return index.h("figure", { itemtype: "http://schema.org/ImageObject", class: this.active ? 'loaded' : '', onClick: () => { this.zoom && this.zoom.open(); } }, index.h("div", { class: "overlay" }), index.h("picture", null, this.renderPicture()), index.h("div", { class: "placeholder", style: { "background-image": `url(${this.poster})` } }), index.h("stellar-intersection", { element: this.element, in: this.in.bind(this) }));
        }
        else {
            return index.h("stellar-intersection", { element: this.element, in: this.in.bind(this) });
        }
    }
