...
const index$1 = require('./index-9b3c8604.js');
require('./index-9d31ce96.js');
const theme = require('./theme-9468c362.js');
const matchPath = require('./match-path-1773ac12.js');
const domUtils = require('./dom-utils-fb324327.js');
const activeRouter = require('./active-router-f27eab3f.js');
...
const isValid = (elm) => {
    if (elm.nodeType === 1) {
        ...
        for (let i = 0; i < elm.attributes.length; i++) {
            ...
            if (index$1.isStr(val) && val.toLowerCase().indexOf('on') === 0) {
                return false;
            }
        }
...
const Icon = class {
    ...
    loadIcon() {
        ...
        if (!this.ariaLabel) {
            const label = index$1.getName(this.name, this.icon, this.mode, this.ios, this.md);
    ...
    render() {
        ...
        return (index.h(index.Host, { role: "img", class: Object.assign({ [mode]: true }, createColorClasses(this.color), { [`icon-${this.size}`]: !!this.size, 'flip-rtl': !!flipRtl && this.el.ownerDocument.dir === 'rtl' }) }, (( this.svgContent)
...
const getIonMode = (ref) => {
    return index.getMode(ref) || document.documentElement.getAttribute('mode') || 'md';
};
