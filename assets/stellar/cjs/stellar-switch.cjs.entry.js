...
const Switch = class {
    ...
    render() {
        return (index.h("label", { class: "label", htmlFor: this.name, onClick: () => { this.activate(); } }, index.h("input", { type: "checkbox", name: this.name, id: this.name, checked: this.checked, tabindex: "-1", onClick: () => { this.activate(); } }), index.h("button", { type: "button" }, index.h("span", null, index.h("ion-icon", { name: this.checked ? "checkmark" : "close" }))), index.h("slot", null)));
    }
