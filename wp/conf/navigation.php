 ...
 * Custom walker class.
 */
class SPLITINFINITIES_Walker_Tabs extends Walker_Nav_Menu {
 ...
 * Custom walker class.
 */
class SPLITINFINITIES_Walker_Anchors extends Walker_Nav_Menu {
    /**
     * Starts the list before the elements are added.
     *
     * Adds classes to the unordered list sub-menus.
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        // Build HTML output and pass through the proper filter.
        $item_output = sprintf( '%1$s<li><a%2$s>%3$s%4$s%5$s</a></li>%6$s',
            $args->before,
            $attributes,
            $args->link_before,
            apply_filters( 'the_title', $item->title, $item->ID ),
 ...
 * Custom walker class.
 */
class SPLITINFINITIES_Walker_Social extends Walker_Nav_Menu {
    /**
     * Starts the list before the elements are added.
     *
     * Adds classes to the unordered list sub-menus.
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $item_output = sprintf( '%1$s<li><a%2$s target="_blank" rel="noopener">%3$s<ion-icon src="/wp-content/themes/'.get_template().'/assets/vector/%4$s.svg" class="black fs6"></ion-icon>%5$s</a></li>%6$s',
            ...
            $attributes,
            $args->link_before,
            apply_filters( 'the_title', strtolower($item->title), $item->ID ),
            $args->link_after,
            $args->after
        );
        $output .= $item_output;
 ...
 * Custom walker class.
 */
class SPLITINFINITIES_Walker_SocialText extends Walker_Nav_Menu {
    /**
     * Starts the list before the elements are added.
     *
     * Adds classes to the unordered list sub-menus.
